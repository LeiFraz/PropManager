...
class UserServices {
    ...
    public getClaimsByUser = async(req: Request, res:Response):Promise <any> =>{
        const id_user = parseInt(req.params.id);
        const subrec= req.params.quejas;
        console.log(req.params);
        
        const isValidId = this.helperU.verifyId(id_user);
        if(subrec !== 'quejas' || isNaN(id_user) || !isValidId){
            return res.status(404).json({error: "Bad request."});
        }

        try {
            const claimsUser= await this.controller.getClaimsUser(id_user);
            if(!claimsUser){
                return res.status(400).json({error: "No se encontraron resultados para la busqueda."})
            }

            
            res.status(200).json(claimsUser);

        } catch (error) {
            res.status(500).json({error:"No se pudo procesar su consulta."});
        }
